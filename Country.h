...
public:
	...
	// The value of the enum elements are the ISO numeric code for each country. Note, NOCOUNTRY, XXX, EUR, XAF, XCD, XOF, XPF, NUMCOUNTRY, and MAXCOUNTRY are not ISO codes
	...
	static Country
	countryIndex(const int i)  { return CountryCode(m_toISO[i]); }

    static int
	countryIndex(const Country c) { return CountryCode(m_fromISO[c]); }
